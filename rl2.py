...
def raise_level():
    ...
    for words in char_dict:
        if words.isdigit():
            sklst=[]
            def skills_to_list(letter):
                if char_dict[words][0].startswith(letter):
                    sklist.append([char_dict[words][0],char_dict[words][1],char_dict[words][2],char_dict[words][3]])
                    sklst=sorted(sklist, key=lambda skill: skill[1])
                    print sklst
                return sklst
            sk=1
            if ska == 1:
